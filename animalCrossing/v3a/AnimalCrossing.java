package animalCrossing.v3a;

import java.util.ArrayList;
import java.util.Scanner;

import animalCrossing.v3a.Animal.Color;
import animalCrossing.v3a.Animal.Species;
...
public class AnimalCrossing {
	...
	static ArrayList<ArrayList<Animal>> allSolutions = new ArrayList<>(20);
...
//		printSolution();
	...
	private static void findSolutions(ArrayList<ArrayList<Animal>> board,
		...
		if (pickedAnimals.size() == board_size * board_size) {
			ArrayList<Animal> solution = new ArrayList<Animal>(pickedAnimals);
			allSolutions.add(solution);
		}

		for (int i = 0; i < board_size; i++) {
			if (nextAnimalsInRows.get(i).isMatch(animalToRemove)) {
				findSolutions(board, nextAnimalsInRows.get(i), i);
			}
		}

		pickedAnimals.remove(animalToRemove);
		if (animalToRemove != null && animalToRemove.getColor() != Color.DONE)
			board.get(row).add(0, animalToRemove);
		if (animalToRemove != null && animalToRemove.getColor() == Color.DONE) {
			board.get(row).add(animalToRemove);
		}
	}
